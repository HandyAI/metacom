    ...
    if (req.url === '/api') {
      ...
      body.then((data) => {
        ...
        if (data.length > 0) {
          ...
          if (!args && Buffer.isBuffer(data)) {
            const buffer = Buffer.from(data, "hex");
            const urlEncodedString = buffer.toString();
            const decodedString = decodeURIComponent(urlEncodedString);
            const obj = Object.fromEntries(new URLSearchParams(decodedString));
            args = obj;
          }

          if (!args) {
            const error = new Error("Args parsing error");
